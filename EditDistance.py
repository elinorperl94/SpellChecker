...
def distance_by_errors(a,b):
   ...
   for i in range (0,min_length-1):
        if a[i]==b[i+1] and a[i+1] ==b[i]:
            ...
            fixed_a_list[i] = b[i]
            fixed_a_list[i+1] = b[i+1]
   a = "".join(fixed_a_list)
   without_transpose =list (levenshtein_ids(a,b))

   without_transpose.insert(0,transpose_count)
   return numpy.array(without_transpose)
